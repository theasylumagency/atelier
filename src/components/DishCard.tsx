    ...
    return (
        <div className="group relative flex flex-col border border-white/10 glass-card bg-black/40 text-stone-100 transition-all hover:border-white/30 h-full">
            ...
            <div className="relative h-56 w-full overflow-hidden border-b border-white/10 bg-zinc-900/50 flex-shrink-0">
                {dish.photo?.small ? (
                    <img
                        src={`https://images.unsplash.com/photo-1546069901-ba9599a7e63c?q=80&w=600&auto=format&fit=crop`} // Fallback to placeholder image as we don't have the actual images in public/images
                        alt={dish.title.en}
                ...
                {onEdit && (
                    <button
                        onClick={() => onEdit(dish)}
                        className="absolute top-3 right-3 border border-white/20 bg-black/60 px-3 py-1.5 text-[9px] uppercase font-sans tracking-[0.3em] text-stone-300 backdrop-blur-sm transition-all hover:bg-white hover:text-black md:opacity-0 md:group-hover:opacity-100 focus:opacity-100"
                    >
                        {dict.panel.editAsset || 'Edit Asset'}
                    </button>
                )}
            </div>
            ...
            <div className="flex flex-1 flex-col justify-between p-6">
                <div>
                    ...
                    <p className="text-sm font-sans text-stone-400 font-light leading-relaxed line-clamp-3">
                        {dish.description?.ka || dish.description?.en || ''}
                    </p>
                </div>

                {/* The Action Floor (Toggles) */}
                <div className="mt-8 flex flex-col gap-4 border-t border-white/10 pt-5">
